...
struct process {
    ...
    ulonglong turnaround_time; // time to terminate
    ulonglong response_time; // time to schedule for 1st time
    ulonglong cpu_time; // time spent running on cpu
    uint n_timer_interrupts;

    //stuff for MLFQ
    uint queue_level;
    ulonglong remaining_runtime; // on current queue-level
