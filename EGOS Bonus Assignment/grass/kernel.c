...
static void intr_entry(uint id) {
    ...
    if (curr_status != PROC_UNUSED) {
        proc_set[curr_proc_idx].n_timer_interrupts++;
    }
...
static void proc_yield() {
    ...
    static ulonglong last_sched_time = 0; // Tracks start of current slice

    // Update stats for the OLD process (if we aren't just starting up)
    if (last_sched_time != 0 && curr_proc_idx != 0) {
        ulonglong runtime = current_time - last_sched_time;
        proc_set[curr_proc_idx].cpu_time += runtime;
        
        // Call mlfq_update_level to check if process queue level needs to be updated
        mlfq_update_level(&proc_set[curr_proc_idx], runtime);
    }

    // Check for Rule 5 (resetting all processes to queue 0)
    mlfq_reset_level();


    int next_idx = MAX_NPROCESS;

    uint best_level = 5; //taking any value larger than 4
    ...
    for (uint i = 1; i <= MAX_NPROCESS; i++) {
        ...
        if (p->status == PROC_READY || p->status == PROC_RUNNABLE) {
            // Checking if queue level is lower than best level
            if (p->queue_level < best_level) {
    ...
    // If no runnable process found, set it to 0
    ...
    if (next_idx < MAX_NPROCESS) {
        ...
        // if respone time == -1 (default value), update response time
        if (proc_set[next_idx].response_time == (ulonglong)-1) {
            proc_set[next_idx].response_time = current_time - proc_set[next_idx].start_time;
        }

        // Reset the slice timer for the NEXT process
        last_sched_time = mtime_get();
